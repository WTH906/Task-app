...
export function FileAttachment({
  ...
  const handleRemove = async () => {
    ...
    if (urlParts[1]) await supabase.storage.from("task-files").remove([urlParts[1]]);
    onRemoved();
  };
    ...
    return (
      <div className="flex items-center gap-1.5 bg-surface3 rounded px-2 py-1 text-xs group">
        ...
        <a href={fileUrl} target="_blank" rel="noopener noreferrer"
          className="text-violet2 hover:text-violet truncate max-w-[120px]" title={fileName}>{fileName}</a>
