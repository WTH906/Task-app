...
export function ActiveTimerBadge({ userId }: { userId: string }) {
  ...
  // Re-check when timer starts/stops anywhere in the app
  useEffect(() => {
    const handler = () => setTimeout(check, 300); // small delay for DB write to complete
    window.addEventListener("timer-changed", handler);
    return () => window.removeEventListener("timer-changed", handler);
  }, [check]);

  // Safety net for timers started in another tab. Same-tab changes already
  // arrive via the "timer-changed" event above, so this doesn't need to be
  // fast — at 10s it issued 2-4 queries every 10 seconds on every page in the
  // app, even with no timer running.
  useEffect(() => {
    const i = setInterval(() => {
  ...
  useEffect(() => {
    ...
    const calcElapsed = () => timer.baseElapsed + Math.round((Date.now() - timer.startedAt) / 1000);
