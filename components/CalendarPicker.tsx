...
export function CalendarPicker({ value, onChange, className = "", label, variant = "date" }: CalendarPickerProps) {
  ...
  /*
    "Work on" (date_key) and "Due" (deadline) are the app's central concept and
    used to render as two adjacent chips of identical size and shape,
    distinguished only by a small icon — so nobody could tell them apart.

    They are now named, and visually distinct:
      · Work on — filled chip, calendar icon
      · Due     — outlined chip, timer icon

    When empty they collapse to a faint "+ Work on" / "+ Due" rather than a
    full-weight "No date" chip that carries the same visual weight as real
    data while saying nothing.
  */
  const isDeadline = variant === "deadline";
  const name = label ?? (isDeadline ? "Due" : "Work on");
  const filled = !!value;
  ...
  return (
    <div className={`inline-block ${className}`}>
      <button
        ...
        aria-label={filled ? `${name}: ${value}` : `Set ${name.toLowerCase()} date`}
        title={isDeadline ? "When it's due" : "When you plan to work on it"}
        className={
          filled
            ? `flex items-center gap-1 text-xs px-2 py-1 rounded transition-colors ${
                isDeadline
                  ? "border border-amber/50 text-amber hover:border-amber bg-transparent"
                  : "bg-surface3 hover:bg-border text-txt2 hover:text-txt"
              }`
            : "flex items-center gap-1 text-xs px-1.5 py-1 rounded text-txt3 hover:text-txt2 border border-dashed border-border hover:border-border2 transition-colors"
        }
      >
        {isDeadline ? <Timer size={12} /> : <CalendarDays size={12} />}
        <span className={filled ? "" : "opacity-80"}>
          {filled ? <><span className="text-txt3 mr-1">{name}</span>{value}</> : name}
        </span>
      </button>
      {popup}
    </div>
