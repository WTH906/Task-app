...
import { fireAndForget } from "@/lib/db-helpers";
...
export function useTimer({ userId, projectId, tasks, onElapsedChange, onTaskUpdate }: UseTimerOptions) {
    ...
    if (!activeTaskId.startsWith("sub:")) {
      ...
      if (task && task.est_minutes > 0 && !task.alarm_fired_at) {
        ...
        if (live >= threshold) {
          ...
          fireAndForget(
            supabase.from("project_tasks").update({ alarm_fired_at: firedAt }).eq("id", activeTaskId),
            "timer:alarm"
          );
          onTaskUpdate(activeTaskId, { alarm_fired_at: firedAt });
        }
      }
    }
  ...
  const startTimer = useCallback(async (timerId: string) => {
    ...
    const table = isSub ? "subtasks" : "project_tasks";
    fireAndForget(
      supabase.from(table).update({ timer_started_at: now.toISOString() }).eq("id", id),
      "timer:start"
    );
  ...
  const stopTimer = useCallback(async () => {
    ...
    const table = isSub ? "subtasks" : "project_tasks";
    fireAndForget(
      supabase.from(table).update({ elapsed_seconds: finalElapsed, timer_started_at: null }).eq("id", id),
      "timer:stop"
    );

    const entity = isSub ? sub : task;
    if (sessionTime > 5 && entity) {
      const detail = isSub
        ? `↳ ${entity.name} — ${formatSeconds(sessionTime)} tracked`
        : `${entity.name} — ${formatSeconds(sessionTime)} tracked`;
      logActivity(supabase, userId, projectId, "Timer stopped", detail);
      fireAndForget(
        supabase.from("time_logs").insert({
          user_id: userId,
          project_id: projectId,
          task_id: isSub ? task?.id || null : id,
          subtask_id: isSub ? id : null,
  ...
  // Auto-save every 5s + save on visibility hidden + tick on visible
  useEffect(() => {
    if (!activeTaskId) return;
    ...
    const handleVisibility = () => {
      if (document.visibilityState === "hidden") {
        ...
        // Tab became visible again — immediately update the display
