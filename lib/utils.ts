...
/** Legacy "[Project]" prefix on planner rows created before migration v21. */
export const LEGACY_TAG_RE = /^\[[^\]]*\]\s*(?:↳\s*)?/;

export const PRIORITY_COLORS: Record<number, string> = {
  1: "#4ade80", 2: "#34d399", 3: "#eab308", 4: "#f97316", 5: "#ef4444",
};

export function deadlineColor(targetDatetime: string, now: Date): string {
  const diff = new Date(targetDatetime).getTime() - now.getTime();
