...
export async function fetchOverdueTasks(
  ...
  const { data: tasks, error } = await supabase
    ...
    .lt("deadline", today)
    .lt("progress", 100)
    // Repeating tasks are never "overdue". Their `progress` is pinned at 0 by
    // design (completion lives in project_task_checks — see migration v23), so
    // without this they'd match `progress < 100` forever and sit on the
    // overdue list climbing past "30 days late", with no control anywhere in
    // the UI able to clear them.
    .is("recurrence", null)
...
export async function fetchTaskDeadlines(
  ...
  const { data, error } = await supabase
    ...
    .eq("user_id", userId)
    .is("archived_at", null)
    // Same reason as fetchOverdueTasks: a repeating task's `progress` is not
    // its completion, so the deadlines countdown can't reason about it and
    // would show it as permanently outstanding.
    .is("recurrence", null)
    .not("deadline", "is", null);
