...
export function cleanDeadline(d: unknown): string | null {
  ...
  // Try parsing.
  // NOTE: must use local date parts, not toISOString(). In UTC+ timezones
  // toISOString() rolls midnight back to the previous day, so every imported
  // non-ISO deadline landed a day early.
