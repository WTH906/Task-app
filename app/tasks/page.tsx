...
export default function TaskListPage() {
  ...
  // Quick capture writes straight to quick_tasks; refresh when it does.
  useEffect(() => {
    const handler = () => loadTasks();
    window.addEventListener("quick-tasks-changed", handler);
    return () => window.removeEventListener("quick-tasks-changed", handler);
  }, [loadTasks]);
  ...
  const addTask = async () => {
    ...
    if (error || !newTask) { toast("Failed to add task", "error"); return; }
  ...
  const updateTask = async (id: string, field: string, value: string | number | null) => {
    ...
    const { error } = await supabase.from("quick_tasks").update({ [field]: dbValue }).eq("id", id);
    if (error) { toast("Failed to save change", "error"); return; }
  ...
  return (
    <div className="page-shell page-shell--list animate-fade-in">
