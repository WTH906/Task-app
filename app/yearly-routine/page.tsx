...
export default function YearlyRoutinePage() {
  ...
  const [formDayFrom, setFormDayFrom] = useState<number | null>(null);
  ...
  const saveTask = async () => {
    ...
    if (editingTask) {
      await supabase.from("yearly_routine_tasks")
        .update({ text: formText.trim(), est_minutes: formEst, month_from: formMonthFrom, month_to: formMonthTo, day_from: formDayFrom })
        .eq("id", editingTask.id);
      setTasks(prev => prev.map(t => t.id === editingTask.id
        ? { ...t, text: formText.trim(), est_minutes: formEst, month_from: formMonthFrom, month_to: formMonthTo, day_from: formDayFrom } : t));
    } else {
      const { data: newTask } = await supabase.from("yearly_routine_tasks").insert({
        user_id: userId, text: formText.trim(), est_minutes: formEst,
        month_from: formMonthFrom, month_to: formMonthTo, day_from: formDayFrom, sort_order: tasks.length,
      }).select().single();
  ...
  const closeModal = () => {
    ...
    setFormText(""); setFormEst(0); setFormMonthFrom(null); setFormMonthTo(null); setFormDayFrom(null);
  };

  const openEdit = (task: YearlyRoutineTask) => {
    setEditingTask(task); setFormText(task.text); setFormEst(task.est_minutes);
    setFormMonthFrom(task.month_from); setFormMonthTo(task.month_to); setFormDayFrom(task.day_from);
    setModalOpen(true);
  };

  const openAdd = () => {
    setEditingTask(null); setFormText(""); setFormEst(0);
    setFormMonthFrom(null); setFormMonthTo(null); setFormDayFrom(null); setModalOpen(true);
  };
  ...
  const handleDragEnd = async () => {
    ...
    if (!userId) return;
    const supabase = createClient();
    // The previous forEach built N query builders and threw them away.
    // Those are lazy thenables — never awaited means never sent, so the new
    // order was silently lost on reload.
    const { error } = await reorderRows(
      supabase, "yearly_routine_tasks", tasks.map((t) => t.id), userId
    );
    if (error) console.error("[yearly-routine:reorder]", error);
  };
    ...
    <div className="page-shell page-shell--list">
            ...
            <div key={task.id}
              ...
              {(task.month_from || task.month_to) && (
                <span className="text-xs text-violet2 bg-violet/10 px-2 py-0.5 rounded-md font-medium shrink-0">
                  {task.month_from && task.month_to
                    ? `${task.day_from ? task.day_from + " " : ""}${MONTHS[task.month_from - 1].slice(0, 3)} – ${MONTHS[task.month_to - 1].slice(0, 3)}`
                    : task.month_from
                    ? `${task.day_from ? task.day_from + " " : ""}${MONTHS[task.month_from - 1].slice(0, 3)}`
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editingTask ? "Edit Yearly Task" : "Add Yearly Task"}>
        <div className="space-y-4">
          ...
          {formMonthFrom && (
            <div>
              <label className="block text-sm text-txt2 mb-1.5">Day in {MONTHS[formMonthFrom - 1]} (for scheduler)</label>
              <input type="number" min={1} max={31} value={formDayFrom || ""} onChange={e => setFormDayFrom(parseInt(e.target.value) || null)}
                placeholder="e.g. 15"
                className="w-20 glass-field px-3 py-2 text-txt text-sm" />
              <p className="text-[10px] text-txt3 mt-1">
                The task will appear in the day scheduler on this day each year
              </p>
            </div>
          )}
